...
public class VNPayQuery {
    ...
    public VNPayQuery(
            ...
            @Qualifier("vnpayWebClient") WebClient webClient,
                    ...
                    .exchangeToMono(response -> response.bodyToMono(String.class)
                            ...
                            .map(body -> {
                                ...
                                auditLogService.logOutbound(
                                        ...
                                        body,
                                        durationMs,
                                        response.statusCode().isError() ? "HTTP " + status : null
                                );
                                if (response.statusCode().isError()) {
                                    throw new IllegalStateException("VNPAY QueryDR failed: HTTP " + status);
                                }
                                return body;
                            }))
                    .block(); // đồng bộ
